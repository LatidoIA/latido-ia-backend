...
app = FastAPI(
    title="Latido IA API",
    version="1.0.0",
    description="Procesamiento de audio de latidos + gestión de cuidadores y métricas"
)
...
async def analizar_audio(
    ...
    try:
        ...
        os.remove(raw_tmp)
        return JSONResponse({"error": f"Conversión a WAV fallida: {e}", "waveform_png": None}, status_code=500)
    finally:
    ...
    try:
        ...
        peaks, _ = find_peaks(env_smooth, distance=sr*0.4, height=np.mean(env_smooth)*1.2)
        ...
        tb = traceback.format_exc()
        return JSONResponse({"error": tb, "waveform_png": None}, status_code=500)
    finally:
